...
func TestE2EFaultInjection(t *testing.T) {
	...
	// storage.
	token, err := auth.MintAccessToken(ts.srv.authStore.SigningKeyMaterial(), "https://alice.example.com/profile/card#me", []string{"rw"}, auth.AccessTokenTTL, "https://id."+ts.srv.cfg.Domain, ts.srv.cfg.Audience)
	if err != nil {
